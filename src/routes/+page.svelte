<script lang="ts">
	...
	import { ocrLiteratureSelectionSummary } from '$lib/englishLiteratureHub';
		...
		return subjectsToShow
			.map((subject) => {
				...
				if (subject.subject === 'English Literature' && subject.board === 'OCR') {
					const selections = data.learnerSettings?.englishLiteratureSelections;
					return {
						...entry,
						href: resolve('/english-literature'),
						meta: 'OCR J352 · your course',
						detail: selections
							? ocrLiteratureSelectionSummary(selections)
							: 'Choose the four options your school teaches.',
						questionActionLabel: 'Your questions'
					};
				}
				return {
					...entry,
	...
	function homeProfileSubjectMeta(
		subject: NonNullable<PageProps['data']['learnerSettings']>['subjects'][number]
	) {
	...
	{#if data.user}
		...
		>
			<div class="qc-home-section-head">
				...
				<p>Start with flashcards, MCQ or exam questions for the subjects in your profile.</p>
				...
				{#each signedInSubjectEntryPoints as subject (subject.label)}
					<article>
						...
						<div class="qc-home-subject-actions" aria-label={`${subject.label} practice choices`}>
							...
							{/if}
							<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
							<a href={subject.href}>{subject.questionActionLabel}</a>
						</div>
					</article>
				{:else}
					<article>
