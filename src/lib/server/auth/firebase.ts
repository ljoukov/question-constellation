import { getRuntimeEnv, type RuntimeEnv } from '$lib/server/env';
import { responseErrorAsString } from '$lib/server/http';
...
export async function verifyFirebaseIdToken(
	...
	if (token.auth_time > now) {
		throw new Error('Auth time is in the future');
	}
	if (token.sub !== token.user_id) {
		throw new Error('Token subject does not match user_id');
	}
