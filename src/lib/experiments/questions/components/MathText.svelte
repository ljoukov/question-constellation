	...
	function inlineSegments(value: string, strong = false): Segment[] {
		...
		while ((match = pattern.exec(value))) {
			if (match.index > cursor) {
				segments.push({ kind: 'text', text: value.slice(cursor, match.index), strong });
			}
			if (match[1] !== undefined) {
				...
				segments.push(...inlineSegments(match[2] ?? '', true));
			}
			cursor = match.index + match[0].length;
		}

		if (cursor < value.length) {
			segments.push({ kind: 'text', text: value.slice(cursor), strong });
		}
