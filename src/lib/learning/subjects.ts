...
const subjectBySlug = new Map(
	Object.entries(subjectSlugByName).map(([subject, slug]) => [slug, subject as SupportedLearnerSubject])
);

export function learnerSubjectSlug(subject: string): string {
	return subjectSlugByName[subject as SupportedLearnerSubject] ?? subject.toLowerCase().replace(/[^a-z0-9]+/g, '-');
}

export function learnerSubjectFromSlug(slug: string | null | undefined): SupportedLearnerSubject | null {
	if (!slug) return null;
...
export function isScienceLearnerSubject(subject: string): subject is 'Biology' | 'Chemistry' | 'Physics' {
