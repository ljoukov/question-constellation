# Adding a GCSE Subject

This is the canonical runbook for extending Question Constellation to another subject, exam board,
specification, or materially different paper family. Point Codex at this document when asking it to
build a new import pipeline.

The intended result is a repeatable source-to-publication workflow. A successful first import is not
enough if it depends on hand-edited JSON, one-off D1 changes, or instructions that exist only in a
Codex thread.
...
The more detailed documents remain authoritative. This guide explains how to apply them when adding a
subject.
...
A one-paper canary proves plumbing and quality for that paper. It does not prove that the subject,
qualification, or curriculum is fully covered.

## The Architecture to Extend
...
flowchart TD
  ...
  L --> M["Database coverage checks and public route crawl"]
```
...
When a canary exposes a defect:

1. Add or improve a shared prompt rule, normalizer, schema, renderer, validator, repair operation, or
   regression fixture.
2. Rerun from the official source material.
3. Keep the failed and repaired summaries as evidence.
...
- weaken a gate to make one paper pass; or
- treat a stale passing judge report as approval for changed data.
...
## 1. Declare Scope and Acceptance Before Coding

Record the exact launch cohort:
...
the declared cohort, not merely the easiest paper:
...
Choose a small canary cohort that covers the matrix. A second paper should deliberately test
cross-paper answer-chain reuse.
...
## 3. Build a Hash-Bound Assessment Manifest

Create a deterministic downloader/discovery adapter or a versioned manifest for the new corpus.
...
## 4. Prove the Shared Data Model Can Represent the Subject
...
For each new task shape, verify the complete path:
...
## 5. Run a Canary Through the Real Automated Loop
...
strict audits, solvability, and a D1 conflict-check dry run, but does not write D1 or R2.
...
The current recall compiler is deliberately limited to Biology, Chemistry, and Physics and its
current durable bundle requires exactly four recognition choices. Adding another subject requires an
explicit cross-cutting extension to its subject allowlists, card taxonomy, prompt, evidence model,
independent review, bundle validator, importer, UI, and tests. Do not smuggle that feature into a
...
If variable three- or four-choice recognition is desired, implement it as a separate versioned
bundle/schema/UI migration. Until then, the current four-choice artifact contract is authoritative.
...
3. Verify post-write question, overlay, mark-row, checklist, model-answer, answer-key, and chain
   coverage.
4. Crawl every public route and asset for each imported source document:
...
5. Re-run the same import in dry-run/idempotency mode and confirm it proposes no unexplained
   mutation.
6. Return the coverage ledger, artifact paths, migration/import result, commit, and public-route
   evidence.
...
A subject rollout is done only when:

- the exact declared curriculum and paper cohort are recorded;
...
```text
Add <SUBJECT / BOARD / SPECIFICATION> to Question Constellation.

Treat docs/adding-a-subject.md as the runbook and acceptance criteria. Read AGENTS.md,
...
First declare the exact curriculum and assessment cohort and produce a coverage ledger. Extend the
existing curriculum catalog, source manifest, shared production extraction orchestrator, importer,
renderers, grading, solvability, and product registries. Do not create a parallel one-off importer,
hand-edit accepted artifacts, patch D1 rows, or weaken publication gates.

Run a small representative canary without production writes. Turn every canary defect into a shared
prompt, schema, normalizer, renderer, validator, repair operation, or regression fixture, then rerun
from the official sources. Test cross-paper chain reuse in a later wave and keep explicit denominators
for papers, questions, marks, response kinds, grading evidence, chains, routes, curriculum targets,
and any held-out material.
...
public route, and every referenced asset.
```
