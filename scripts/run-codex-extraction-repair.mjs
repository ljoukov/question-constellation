...
7. For Ordnance Survey map extracts and other grid-reference maps, repair source assets by using rendered official-page crops that include the complete map frame plus readable eastings and northings on the relevant margins. Verify every referenced grid square, coordinate label, scale bar, north arrow, and measurement endpoint mentioned by the failed subquestions. A crop is still defective if it omits a northing/easting needed to locate a grid square such as 0870, 0970, or 7109, or clips a labelled point such as X/Y used for distance or width measurement.
8. If a task refers to an existing hypothesis, restore the exact hypothesis in learner-visible stem/lead blocks. If pressure changes a reaction-specific equilibrium/yield, restore the exact reversible equation with coefficients/state symbols in a learner-visible equation block.
9. Equation-completion tasks require equation-blanks; multiple named written fields such as Test and Result require labeled-lines. Strip Additional page, margin, copyright/footer, turn-over/end-of-paper, and neighboring-question text. Adjacent-page context/assets must be relevant, explicitly bound to the learner surface, visually verified, and review-blocked until proven.
10. Use bash pdf-tools.sh for PDF observation commands such as pdf-info, pdftotext-pages, render-pages, crop, crop-page, and line-count. helper.mjs is the deterministic validator/normalizer; do not call helper.mjs pdf-info or helper.mjs pdftotext-pages.
11. Run deterministic validation with:
