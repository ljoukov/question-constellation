...
export function buildFullPaperPrompt({
	...
	return [
		...
		'If a question refers to an existing hypothesis (for example "their hypothesis", "the hypothesis", or whether it is supported), copy the exact source hypothesis into learner-visible contextBlocks/stemBlocks as well as selfContainedPromptText. Do not publish a question that asks about a hidden hypothesis.',
		'For a reaction-specific question about how pressure changes equilibrium position or product yield, include the exact reversible equation with its coefficients/state symbols or other explicit molecule-count evidence in a learner-visible equation block. The task is not standalone if the equation exists only in a sibling question or mark scheme.',
		...
		'Do not copy exam-booklet furniture into promptText, selfContainedPromptText, contextText, or render blocks. Strip Additional page text, question-number margin instructions, copyright/footer lines, Turn over, END OF QUESTIONS, and neighboring-question material.',
...
export function deterministicCandidateIssues(candidate, options = {}) {
	...
	for (const question of candidate.questions ?? []) {
		const issues = [
			...
			...learnerFacingExtractionIssues(question),
